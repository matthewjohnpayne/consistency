...
def search_for_cross_designation_duplicates():
    ...
    list_of_dicts = dask.compute([ read_into_bit_dict(f) for f in list(file_dict.values())[:10] ])
        
    print('...', len(list_of_dicts), [type(_) for _ in list_of_dicts], time.time()-start )
    sys.exit()
    list_of_dup_dicts = []
    ...
    for tup in list_of_lists:
        ...
        for k in range(0,len(lst),chunk):
            ...
            sys.exit(f'End of chunk: len(list_of_dup_dicts_for_i[-1])={len(list_of_dup_dicts_for_i[-1])}')
